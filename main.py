...
class popupwin(object):
    ...
    def cleanup(self):
        ...
        access = "" #Here you need to put the password that you want to use / Aquí colocarás la contraseña que deseas usar
        if self.value == access:
            self.loop = True
            self.top.destroy()
            window.deiconify()
        #IF THE PASSWORD IS NOT CORRECT THIS WILL AUMENT BY 1 THE NUMBER OF FAILED ATTEMPS UNTIL THE NUMBER IS 5
        #SI LA CONTRASEÑA NO ES CORRECTA EL NUMERO DE INTENTOS FALLIDOS SERÁ AUMENTADO EN 1 HASTA QUE SEA 5
        else:
            self.attempts += 1
            if self.attempts == 5:
                window.quit()
...
#This will add the encrypted data to the text file
#Esto añadira los encriptados al documento de texto
class entity_add:
    ...
    def write(self):
        ...
        for letter in n:
            if letter == " ":
                ...
                encryptedN += chr(ord(letter)+8)
        for letter in e:
            if letter == " ":
                encryptedE += " "
            else:
                encryptedE += chr(ord(letter)+8)
        for letter in p:
            if letter == " ":
                encryptedP += " "
            else:
                encryptedP += chr(ord(letter)+8)
...
class entity_display:
    def __init__(self, master, n, e, p, i):
        ...
        self.label_name = Label(self.window, text=decryptedN, font=("Courier", 14))
        self.label_email = Label(self.window, text=decryptedE, font=("Courier", 14))
        self.label_password = Label(self.window, text=decryptedP, font=("Courier", 14))
...
password_label = Label(window, text="Password: ",font=("Courier", 14))
...
password_label2= Label(window, text="Password: ", font=("Courier", 14))
name_label2.grid(row=5)
email_label2.grid(row=5, column=1)
password_label2.grid(row=5, column=2)
readfile()
window.mainloop()
